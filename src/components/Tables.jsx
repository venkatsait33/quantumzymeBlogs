...
const Tables = ({ onSaveTable, initialTables = [] }) => {
  const [tables, setTables] = useState(initialTables);

  useEffect(() => {
    onSaveTable({ tableData: tables });
  }, [tables]); // Call onSaveTable whenever tables state changes
      ...
      <form className="flex flex-col space-y-4">
        {tables.map((table, tableIndex) => (
          <div key={tableIndex} className="p-4 mb-4 border rounded shadow-sm">
            ...
            <table className="w-full border border-collapse rounded table-auto">
              <thead>
                <tr>
                  {table.headings.map((heading, headingIndex) => (
                    <th
                  ...
                  <th className="px-4 py-2 text-white bg-green-500">Actions</th>
                </tr>
              </thead>
              <tbody>
